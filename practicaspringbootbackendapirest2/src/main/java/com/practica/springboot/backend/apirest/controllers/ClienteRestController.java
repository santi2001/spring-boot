...
public class ClienteRestController {
	...
	public ResponseEntity<?> obtenerClienteEspecifico(@PathVariable Long id)
	{
	...
	public ResponseEntity<?> actualizarCliente(@RequestBody Cliente cliente, @PathVariable Long id)
	{
		...
		{
			response.put("mensaje", "error: no se pudo editar el cliente con el ID ".concat(id.toString().concat( " no existe el cliente en la base de datos")));
			return new ResponseEntity<Map<String,Object>>(response, HttpStatus.NOT_FOUND);
		}
		...
		try
		{
			...
			clienteActual.setCreateAt(cliente.getCreateAt()); // cargamos los datos de esta forma para no perder la referencia del cliente buscado
